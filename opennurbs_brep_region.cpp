...
static bool HasOneComponent(ON_Brep* brep)
{
  if (!brep)
    return true;

  const int Vcount = brep->m_V.Count();
  const int Ecount = brep->m_E.Count();
  int component_count = brep->LabelConnectedComponents();
  for ( int vi = 0; component_count > 1 && vi < Vcount; vi++ )
  {
    int vci = -1;
    const ON_BrepVertex& vertex = brep->m_V[vi];
    for ( int vei = 0; vei < vertex.m_ei.Count(); vei++ )
    {
      int ei = vertex.m_ei[vei];
      if ( ei < 0 || ei >= Ecount )
        continue;
      const ON_BrepEdge& edge = brep->m_E[ei];
      if ( edge.m_edge_user.i <= 0 )
        continue;
      if ( -1 == vci )
      {
        vci = edge.m_edge_user.i;
      }
      else if ( vci != edge.m_edge_user.i )
      {
        // combine these components
        component_count--;
        if ( component_count <= 1 )
          break;
        const int oldi = edge.m_edge_user.i;
        for( ei = 0; ei < Ecount; ei++ )
        {
          if ( oldi == brep->m_E[ei].m_edge_user.i )
            brep->m_E[ei].m_edge_user.i = vci;
        }
      }
    }
  }
...
bool ON_BrepRegionTopology::Create(const ON_Brep* brep)
{
  m_FS.Destroy();
  m_R.Destroy();
  m_brep = const_cast<ON_Brep*>(brep);
  if ( 0 == brep )
    return false;
  const int face_count = brep->m_F.Count();
  if ( face_count <= 0 )
  ...
  m_R.SetCapacity(2);
  m_FS.SetCapacity(2*face_count);
  for ( int fi = 0; fi < face_count; fi++ )
  {
    ON_BrepFaceSide& fs0 = m_FS.AppendNew();
    fs0.m_faceside_index = m_FS.Count()-1;
    ON_BrepFaceSide& fs1 = m_FS.AppendNew();
    fs1.m_faceside_index = m_FS.Count()-1;
    fs0.m_rtop = fs1.m_rtop = this;
    fs0.m_fi = fs1.m_fi = fi;
    fs0.m_srf_dir = 1;
    fs1.m_srf_dir = -1;
    fs0.m_ri = fs1.m_ri = -1;
  }
  // r0 = unbounded region
  // Since m_R[] capacity is 2, the r0 reference will be
  // valid unless we call ON_Brep::m__CreateRegionTop().
  ON_BrepRegion& r0 = m_R.AppendNew();
  r0.m_region_index = m_R.Count()-1;
  r0.m_type = 0;
  bool bHasOneComponent = (0 == ON_Brep::m__RegionTopologyHelper || HasOneComponent(m_brep));
  const int solid_type = bHasOneComponent 
                       ? m_brep->SolidOrientation() 
                       : -99;
  switch(solid_type)
  {
  case 1: // bounded monifold solid - normals point out
  case 2: // bounded manifold solid - normals point in
    {
      // r1 = finte region
      ON_BrepRegion& r1 = m_R.AppendNew();
      r1.m_region_index = m_R.Count()-1;
      r1.m_type = 1;
      r0.m_fsi.Reserve(face_count);
      r1.m_fsi.Reserve(face_count);
      for ( int fi = 0; fi < face_count; fi++ )
      {
        int flip = m_brep->m_F[fi].m_bRev ? 1 : 0;
        if ( 2 == solid_type )
          flip = 1-flip;
        int fsi0 = 2*fi + flip;
        int fsi1 = 2*fi + (1-flip);
        r0.m_fsi.Append(fsi0);
        r1.m_fsi.Append(fsi1);
      }
    }      
    break;
  default:
    {
      if ( bHasOneComponent && m_brep->IsManifold() )
      {
        // open manifold brep with one component
        r0.m_fsi.Reserve(2*face_count);
        for ( int fsi = 0; fsi < 2*face_count; fsi++ )
          r0.m_fsi.Append(fsi);
      }
      else
      {
        // Non manifold case or brep has more than one
        // connected component that could be something
        // like nested spheres.
        if (    0 == ON_Brep::m__RegionTopologyHelper 
             || !ON_Brep::m__RegionTopologyHelper(*m_brep,*this) 
           )
        {
          // non-manifold helper is missing or failed.
          // Just report a single region of undefined type.
          // Using r00 because it's possible that m_R[] was
          // grown and r0 reference is no longer valid
          m_R.SetCapacity(1);
          m_R.SetCount(1);
          ON_BrepRegion& r00 = m_R[0];
          r00.m_fsi.SetCapacity(2*face_count);
          r00.m_fsi.SetCount(0);
          r00.m_type = -1;
          for ( int fsi = 0; fsi < 2*face_count; fsi++ )
            r00.m_fsi.Append(fsi);
        }
      }
    }
    break;
  ...
  // Set region m_brep backpointers, bounding boxes,
  // and face side region indices.
  for ( int ri = 0; ri < m_R.Count(); ri++ )
  {
    ON_BrepRegion& region = m_R[ri];
    region.m_rtop = this;
    region.m_bbox.Destroy();
    for ( int rfsi = 0; rfsi < region.m_fsi.Count(); rfsi++ )
    {
      int fsi = region.m_fsi[rfsi];
      if ( fsi >= 0 && fsi < 2*face_count )
        m_FS[fsi].m_ri = region.m_region_index;
      int fi = m_FS[fsi].m_fi;
      if ( fi >= 0 && fi < m_brep->m_F.Count() )
        region.m_bbox.Union(m_brep->m_F[fi].BoundingBox());
    }
  }
...
{
  return 0 != ON_Brep::m__MergeBrepsHelper 
         ? ON_Brep::m__MergeBrepsHelper(brep0,brep1,tolerance) 
         : 0;
}
